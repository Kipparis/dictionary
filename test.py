...
if args.update:
    ...
    for entry in parse_dict(args.words_file): # must return iterable
        ...
        if entry.category not in previous_header:
            previous_header = entry.category
            category = Category.create(name=previous_header)
        # date, score applied automatically
        Result.create(word=entry.eng_str, category=category)
